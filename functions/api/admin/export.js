...
// ⚠️ حماية من CSV/Formula Injection: بيانات العميل (الاسم/العنوان/البريد) تأتي من
// نموذج الدفع العام بدون أي تصفية لأحرف بداية الخلية — لو بدأت بـ =+-@ فإكسل/جوجل
// شيتس تفسّرها كصيغة (formula) وتُنفَّذ فعليًا عند فتح المالك للملف المُصدَّر.
// نضيف علامة اقتباس ' في البداية (نفس أسلوب Excel/Google Sheets الموصى به) لإجبار
// عرضها كنص خام بدل تنفيذها كصيغة، قبل تهريب علامات الاقتباس المعتاد.
function csvCell(v) {
